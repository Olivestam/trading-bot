...
async def make_trade_decision(df: pd.DataFrame, df_sp: pd.DataFrame):
    ...
    if prediction['prediction'] == 1 and prediction['confidence'] > 0.75:
        ...
        if buy_tracker.possible_buy(symbol):
            buying_capacity = await buy_tracker.get_buying_capacity()
            quantity = int(buying_capacity / stock_price)
            await buy_tracker.record_placed_order(quantity*stock_price)
            await place_buy_order(symbol, quantity, stock_price)
        else:
            print(f"[SKIP BUY] {symbol} - Not enough buying capacity or too many trades today.")
    elif prediction['prediction'] == 2:
        # Sell signal
        stocks_owned = await buy_tracker.get_stocks_owned(symbol)
        if stocks_owned > 0:
